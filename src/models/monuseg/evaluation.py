...
def post_processing(y_pred):
    if type(y_pred) != np.ndarray:
        y_pred_quantized = (y_pred.numpy() == y_pred.numpy().max(axis=-1, keepdims=True)).astype(int)
    else:
        y_pred_quantized = (y_pred == y_pred.max(axis=-1, keepdims=True)).astype(int)
...
def post_processing_simple(y_pred):
    if type(y_pred) != np.ndarray:
        y_pred_quantized = (y_pred.numpy() == y_pred.numpy().max(axis=-1, keepdims=True)).astype(int)
    else:
        y_pred_quantized = (y_pred == y_pred.max(axis=-1, keepdims=True)).astype(int)
    ...
    output = list()
    for s in range(batch_size):
        out = label(np.argmin(y_pred_quantized[s,...,:2],axis=-1))
        output.append(out)
    return np.stack(output, axis=0)
...
def evaluate_equivariance(
    model,
    id_list=None,
    data_path="/home/vincent/python_wkspce/2d_wide_dct_cnn/data/raw/MoNuSeg2018Training",
):
